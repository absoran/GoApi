...
func GetUsersWithMux(w http.ResponseWriter, r *http.Request) {
	fmt.Println("Endpoint Hit: GetUsersWithMux")
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(models.Users)
}

func GetUserByIdWithMux(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	var idParam string = mux.Vars(r)["id"]
	id, err := strconv.Atoi(idParam)
	CheckError(err)
	tempusr := models.User{ID: -1}
	for i := range models.Users {
		if models.Users[i].ID == id {
			tempusr = models.Users[i]
			break
...
func GetUsersByAuthorityWithMux(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	var idParam string = mux.Vars(r)["id"]
	authority, err := strconv.Atoi(idParam)
	CheckError(err)
	var temp = []models.User{}
	switch authority {
	case 0:
	...
	default:
		{
			w.WriteHeader(http.StatusBadRequest)
			fmt.Fprintf(w, "User authority value can be \n0 = admin,1 = merchant,2 = customer,3 = not registered \n else is forbidden")
		}
	}
}

func CreateNewUserWithMux(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	var newUser models.User
	json.NewDecoder(r.Body).Decode(&newUser)
	if newUser.Authority > -1 && newUser.Authority < 4 {
...
func DeleteUserByIdWithMux(w http.ResponseWriter, r *http.Request) {
	fmt.Println("Endpoint Hit: DeleteUserByIdWithMux")
	index := -1
	var idParam string = mux.Vars(r)["id"]
	id, err := strconv.Atoi(idParam)
...
func UpdateUserWithMux(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	fmt.Println("Endpoint Hit: UpdateUserWithMux")
	index := -1
	tempusr := models.User{Authority: -1}
	var idParam string = mux.Vars(r)["id"]
	id, err := strconv.Atoi(idParam)
	CheckError(err)
	json.NewDecoder(r.Body).Decode(&tempusr)
	for i := range models.Users {
		if models.Users[i].ID == id {
			index = i
			break
	...
	if index == -1 {
		fmt.Fprintf(w, "User to update does not exist")
	} else {
		tempusr.ID = id
		if tempusr.Age != 0 {
			models.Users[index].Age = tempusr.Age
